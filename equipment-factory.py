...
class EquipmentFactory:
    ...
    @staticmethod
    def create_from_properties(equipment_type, properties=None):
        """
        Create an equipment model object from generic properties.
        
        Args:
            equipment_type: Type of equipment to create ('light', 'camera', 'set_element')
            properties: Dictionary of properties for the equipment
        ...
        name = properties.get("name", "New Item")
        
        if equipment_type == "light":
            # Create lighting equipment
            light = LightingEquipment(name=name)
            
            # Set position if provided
            if "x" in properties and "y" in properties:
                light.x = properties["x"]
                light.y = properties["y"]
            
            # Set properties
            for key, value in properties.items():
                if hasattr(light, key) and key not in ["id", "created_at", "modified_at"]:
            ...
            if "x" in properties and "y" in properties:
                ...
                camera.y = properties["y"]
            
            # Set properties
            for key, value in properties.items():
                if hasattr(camera, key) and key not in ["id", "created_at", "modified_at"]:
                    setattr(camera, key, value)
            
            return camera
            
        elif equipment_type == "set_element":
            # Extract element type from properties or use default
            element_type = properties.get("element_type", "Wall")
            
            # Create set element
            element = SetElement(name=name, element_type=element_type)
            ...
            # Set properties
            for key, value in properties.items():
                if hasattr(element, key) and key not in ["id", "created_at", "modified_at"]:
